...
summary: Use this skill whenever the user wants to write or organize rough notes, guided thinking, exploratory notes, or messy subject notes that may later become permanent memory.
---

# `cumaru-draft` - maintain rough notes

Load `.cumaru/roles/keeper.md` and `drafts/index.md` before using invocation
arguments to create or promote draft material.
...
6. Set the draft `summary:`, then run `cumaru tree drafts --rows` and `cumaru doctor`.
7. Run `cumaru doctor`.
