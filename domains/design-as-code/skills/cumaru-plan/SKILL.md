...
The plan-lifecycle skill for design-as-code. Covers everything from bootstrap
of a new `plans/<PLAN-ID>/` to readiness-for-absorb. Absorb itself lives in
[`cumaru-absorb`](../cumaru-absorb/SKILL.md).

## Invocation

Load `.cumaru/roles/lead.md`, `plans/index.md`, `specs/index.md`, and
`concepts/index.md`. Use invocation arguments to select the matching recipe and
confirm each judgment before writing.
...
**Role**: Design Lead — the domain's unrestricted single role. Authors
everything, writes handoffs, and drafts deltas.
...
- **Concept selection** — `cumaru-concept` owns pre-plan design directions.
