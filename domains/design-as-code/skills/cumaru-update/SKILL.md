...
5. Apply already validates the staged result with doctor. Optionally run `cumaru doctor --quiet` for a visible post-commit report, then `cumaru tree --deep` when auditing navigation.
...
- Do not create persistent backups; transaction snapshots are private and removed
