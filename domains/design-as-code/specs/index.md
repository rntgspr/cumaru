...
A pillar for the **living spec** of the system — what is true now about foundations, components, journeys, interaction, responsive behavior, accessibility, and content. Authored and refactored by the Design Lead; deltas are absorbed here on plan close.
...
- **Concerns split inside an area.** A large area splits into per-concern files (`<area>/<concern>.md`) discovered through `cumaru tree specs/<area>`. Tasks declare which concerns they touch in their frontmatter `concerns:`.
...
- **Each area is a directory** with `index.md` following `templates/spec.md`, any concern files, and optional subarea directories.
...
- Bootstrapping a new area when planning work that touches an undocumented part of the system (Design Lead).
...
- Pre-plan ideation or open questions → `concepts/<slug>/` or `research/<slug>/`.
