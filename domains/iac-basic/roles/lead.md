...
Each dispatch must name the active changeset and step, topology scope and dependencies, target
environment, allowed files and role permissions, required plan/apply verification, and handoff or
delta-draft expectations. Read and reconcile the handoff before releasing dependent work or close.
...
`intake/` is a tracker mirror authored through `cumaru-intake`; the Lead reads it and preserves tracker provenance.
...
4. **Absorb into `topology/`:** update each affected area's body to the new state.
...
8. Commit the absorption with a message that **names every KEY absorbed** (it is the grep key, and messages survive rebase and squash), then prune the archive directory. The updated `topology/` is the durable record; the commit is how you find it.
...
- **`apps:` values** are environments from `config.yaml` (`apps.values`): `dev` / `staging` / `prod`; `all` for cross-environment/shared (global IAM, DNS, billing); `meta` for framework plumbing only.
