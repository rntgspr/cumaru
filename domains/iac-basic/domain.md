...
> **Axis.** In this domain the universal `targets:` field enumerates **environments** (`dev` | `staging` | `prod` | `all` | `meta`), not software components. A file's `targets:` says which environments it concerns. The field name stays `targets` so `index.md` is byte-identical to the kernel; the semantic lives here. See `config.yaml`'s AXIS NOTE.
...
- **`intake/` — what is asked.** A flat, tracker-agnostic mirror of the tickets/incidents that drive infra change: a provisioning request, a capacity bump, an incident remediation. Each lives at `intake/<KEY>/`, with `type`, scalar `tracker` provenance, and `relates`. There is no project-wide tracker registry.
...
```
intake/ ──┐
          ├──→ plans/ ──absorb──→ topology/
exploring/ ┘                                               │
                                                           │ relates
                                                           ▼
                                                      runbooks/

After absorb: plans/ are removed. Only exploring/ and intake/
entries actually consumed by the closed changeset are cleanup candidates;
shared or unresolved sources remain.
```

Work flows `intake → plans` and is distilled directly into the durable
layer (`topology/`). Guarded cleanup removes its exact consumed transient set.
Only the updated topology and durable runbooks remain as system records;
`git log` is the cross-reference back to the changes that built them.

## Roles

- **Lead (platform)** — primary author of `.cumaru/`. Plans changes, maintains the topology in `topology/`, runs the apply + absorption flow, owns `runbooks/` and `exploring/`, dispatches Dev sub-agents.
...
|-------|-------------------------------------------------------------------------------------------------|-----------|
| Lead  | `plans/index.md`, `topology/index.md`, `intake/index.md`, `runbooks/index.md`  | Orchestrates — needs the full map, including operational procedures. |
| Dev   | none                                                                                            | Operates inside a dispatched `plans/<PLAN-ID>/`. |

### Change-scoped entry

For dispatched work, begin with the active changeset and assigned step. Load
...
evidence and `aux:` inputs, prerequisite handoffs named by `depends-on:`, and
topology candidates named by plan `scope:` or task `concerns:`. Follow only
relevant semantic dependencies; proximity does not authorize unrelated pillar
content.
...
- **Promote across environments.** A change lands in `dev`, then `staging`, then `prod`; the plan records the promotion path, and each artifact's `targets:` scoping says where it has landed.
