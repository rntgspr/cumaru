...
1. **Load the installed `cumaru-topology` skill.** It carries the three recipes: bootstrap, deepen, consolidate. Follow its layout and pre-checks (Lead-only authoring; `depends-on:` IS the apply order).
...
2. **Dispatch by intent.** If `$ARGUMENTS` is:
   ...
   - The user explicitly asks "consolidate", or an area's body reads like a changelog → **consolidate** recipe. Recover history with `git log --follow --oneline -- .cumaru/topology/<path>` when detail is needed; rewrite the area body as a single coherent topology spec stating current state.
