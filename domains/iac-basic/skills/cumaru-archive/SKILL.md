...
End-to-end recipe to close a `plans/<KEY>/` after the change has been applied through its promotion path. It combines `cumaru flow`, `cumaru tree`, and focused health checks. The durable result is the updated `topology/` tree — nothing is carried forward out of `archive/`.
...
Before Phase 0, summarize the changeset's `summary:`, `scope:`, completed task
count, environments reached, and proposed sequence. Ask for `walk` to confirm
between phases or `apply` for one explicit confirmation covering every phase.
Never substitute agent confidence for user confirmation.
...
4. **Exact fit** → that area is the target. **No fit** → create a new area with
   the ``cumaru-topology`` skill, as usual. Never force a claim into an area that
   does not own it.
...
1. Stage + commit: `git add topology/ archive/ plans/` then `git commit -m "chore(.cumaru): absorb <KEY> delta into <stacks>"`. The message **must name every KEY absorbed** — it is the grep key that finds this work later, and messages survive rebase and squash where SHAs do not.
...
3. Run `cumaru tree topology --rows`, `cumaru tree archive --rows`, and `cumaru doctor`.

**Ghost deltas** ("no topology change required"): the archive directory is still pruned and the plan leaves **only its commit**. A change that alters no durable contract has no place in the living topology — that is correct, not a gap.

## Why phased

Phase 1 is non-destructive (copies + frontmatter) — recoverable by deleting `archive/<KEY>/`. Phase 2 removes the source plan (recoverable from git). Phase 4 is final: after the prune, the updated `topology/` is the durable record and the commit is how you find it.
...
| User says | You do |
|---|---|
| "Archive AAA-1234" / "close change X" | Run Phase 0 then all 4 phases on `<KEY>`, confirming the Phase 0 targets, between Phase 1→2, and again before Phase 4 (irreversible prune) |
