...
1. Read `topology/index.md`, then run `cumaru tree topology --rows`. Build the node set from the projected paths and the edge set from each selected area's frontmatter (`depends-on` = solid, `relates` = dashed). Capture each node's `apps` (environments).
