...
summary: Use this skill whenever the user wants to SEE the infrastructure architecture as text — render the topology as a diagram, show what is connected to what, draw the stack/dependency graph, visualize apply order, or map how stacks depend on each other. Trigger on phrases like "draw the architecture", "what's connected where", "show the topology", "diagram the stacks", "render the dependency graph", "diagrama da arquitetura", "o que depende de quê", "como os stacks se conectam". Read-only — it renders the DECLARED topology, it does not change it.
---

# `cumaru-arch` — render the infrastructure topology as a text diagram
...
1. Read `topology/index.md`, then run `cumaru tree topology --rows`. Build the node set from the projected paths and the edge set from each selected area's frontmatter (`depends-on` = solid, `relates` = dashed). Capture each node's `targets` (environments).
...
- This skill is **read-only**. To persist the diagram as a living in-repo artifact (e.g. a generated `topology/architecture.md`), that's a separate, deliberate step the user must ask for — it needs careful placement so `cumaru doctor`'s navigation checks doesn't flag it.
...
| User says | You do |
|---|---|
| "Draw the architecture" / "diagrama da arquitetura" | Read `topology/index.md` + run `cumaru tree topology --rows` → render Mermaid + ASCII to chat |
| "What's connected to `app`?" / "o que depende de `app`" | Prune to `app` + its `depends-on`/`relates`/dependents; render that slice |
| "Show apply order for prod" | Filter nodes whose `targets` include `prod` (or `all`); render the ASCII layering |
