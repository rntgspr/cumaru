...
Infrastructure spikes are **transient** — a provider migration to weigh, a cost-optimization hypothesis, a multi-region sketch. They either mature into a changeset or get dropped. This skill carries the bootstrap recipe plus the two terminal exits (promote / drop). For raw file ops use `cumaru flow`.
...
2. `cumaru flow exploring/<slug> create` → `cumaru flow exploring/<slug>/index.md create`
...
1. **Decide the destination key.** Tracker item exists → `plans/<KEY>/` (run `cumaru intake <KEY>` first if needed). Else → `plans/maintenance-<slug>/`.
...
4. **Remove or carry the spike:** `cumaru flow exploring/<slug> remove`, or `cumaru flow exploring/<slug> copy plans/<PLAN-ID>/exploration.md` first if the notes are worth keeping.
...
- **Changeset authoring** — `cumaru-plan`. **Topology** — explorations never touch `topology/`; only applied changes do, via `cumaru-archive`. **Tracker mirror** — `cumaru-intake` first if you need an issue.
...
| "Drop the opentofu idea" | Confirm → `cumaru flow exploring/move-to-opentofu remove` → run `cumaru tree` |
