...
7. Installs supported slash commands and writes `agent` in config.
...
3. Use `cumaru tag set domain.md components` to write the table (v4 — every body is `[Link, Description]`):
   ...
   ```
4. Update `.cumaru/config.yaml > meta.apps.values` to list those same keys (in addition to the reserved `all` and `meta`).
