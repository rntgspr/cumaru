...
3. Record the required frontmatter, including scalar tracker provenance, status, environment-oriented `targets`, and evidence-backed `relates`.
...
5. Set `targets` from `config.yaml > meta.targets.values`, set a valid `summary`, remove the complete RAW block, then run `cumaru tree intake --rows` and `cumaru doctor --quiet`.

On refresh, source status may change; refined body, type, targets, relations, summary, and existing provenance remain adopter-owned.
