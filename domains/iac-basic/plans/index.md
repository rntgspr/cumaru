...
Active **changesets** — one directory per infrastructure change in flight. Authored by the Lead at planning time; absorbed directly into the durable pillar on close. `Apps` = the environments the change targets.
...
- Closing → Dev writes `delta-draft.md`; Lead finalizes it in place, absorbs into `topology/`, prunes the directory.
