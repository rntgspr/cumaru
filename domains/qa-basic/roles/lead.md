...
Each dispatch must name the active campaign and case, coverage scope and dependencies, target test
levels, allowed files and role permissions, required verification, and handoff or delta-draft
expectations. Read and reconcile the handoff before releasing dependent work or closing the campaign.
...
`intake/` is a tracker mirror authored through `cumaru-intake`. The item's `## Acceptance Criteria (EARS / RFC 2119)` is the requirement your coverage must verify.
...
4. **Absorb into `coverage/`:** update each affected area's body and `## Scenarios (GWT)` to the new state.
...
8. Commit the absorption with a message that **names every KEY absorbed** (it is the grep key, and messages survive rebase and squash), then prune the archive directory. The updated `coverage/` is the durable record; the commit is how you find it.
...
- **`apps:` values** are test levels from `config.yaml` (`apps.values`): `unit` / `integration` / `e2e` / `contract` / `performance`; `all` for cross-level/shared (test utilities, fixtures, CI harness); `meta` for framework plumbing only.
