...
relates: []              # live intake items, durable coverage siblings, or standards to consider
targets: "[unit] | [integration] | [e2e] | [unit, e2e] | [all]   # levels this area is exercised at"
---
...
The behaviours verified here, in Given-When-Then. State the acceptance fact in
durable terms. While intake is live, the scenario may map through `relates:`;
before intake cleanup, retain its stable upstream tracker URL in `## Decisions`
and remove or replace the local relation.
...
- YYYY-MM-DD: short rationale and link to the originating campaign (e.g. `AAA-1234`). Why this level split / runner / fixture strategy over the alternatives.
...
Repository source files this area covers (the test sources) — read by
`cumaru coverage`. Every row targets a source file, resolved from the project
root (never a `.cumaru/` path, a directory, or a URL).

<!-- cumaru:reference -->
| Link | Description |
|------|-------------|
| [<flow>](<tests/path/to/file>) | <one-line prose: what this test file verifies for this area> |
<!-- /cumaru:reference -->
