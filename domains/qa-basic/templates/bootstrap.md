...
depends-on: [] # prerequisite paths under coverage/
relates: [] # intake keys or related coverage areas
targets: [] # test-level keys from config.yaml
---
...
by the `cumaru-coverage` bootstrap (light pass) and grown by future deep passes.
...
coverage paths, `relates` to intake keys or useful non-blocking links, and
`targets` to test-level keys declared in `config.yaml`. This file is working
evidence, not a second durable scenario contract.
...
# Light pass — your job
  ...
  2. Write `coverage/<area>/index.md` following `templates/coverage.md`:
     - frontmatter: `name`, `summary` (one line), `depends-on:` (areas whose
       setup this one presumes), `relates:` (intake `<KEY>`s covered), `targets:`
       (the levels actually exercised, from `config.yaml` `targets.values`).
