...
human_revised: false
targets: [meta]
summary: Framework guidance for Standards and its required workflow.
---


# Standards

**Durable testing conventions** — naming, fixtures/factories, mocking policy per level, coverage gates, flakiness policy, test environments. Unlike the transient pillars (`intake`/`plans`/`absorption`, which distill into `coverage/` then get pruned), standards persist: they are consulted repeatedly and apply ACROSS coverage areas, not to one campaign.
...
- **`targets:`** = which levels the convention applies to (`unit`, `all`, …).
