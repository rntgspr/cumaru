...
4. **Run Phase 1** (non-destructive — copies + frontmatter updates):
   ...
   - For each coverage area in the plan's `scope:`: edit `coverage/<area>/index.md` to reflect the new state, append `$ARGUMENTS` to `deltas:`, and keep `summary:` accurate.
...
6. **Run Phases 3 and 4** — verify with `cumaru tree archive --rows`; commit the coverage absorption; append `SHA | KEY | Description` to `coverage/index.md` `cumaru:absorptions`; prune `archive/$ARGUMENTS/`; commit cleanup; then run `cumaru tree coverage --rows` and `cumaru doctor`.
...
- **No structural rows.** Navigation comes from `cumaru tree`; only the coverage absorption ledger is persisted.
