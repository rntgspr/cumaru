...
the middle of a function body does not. Prose buried in a function body may be versioned with the
source, but it is not independently validated or reachable by the loading rule: nothing relates it
to a concern, and it can drift from the code beneath it until it lies. The framework already has a
home for explanation, and it is not the source file.
...
- Where the domain declares a specification pillar, `cumaru coverage` measures whether tracked
  source has a reference row. It does not prove that the linked prose is accurate or meaningful.
