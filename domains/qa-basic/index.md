...
**Bootstrap and priority.** Every context receives this kernel, `domain.md`, and every installed discipline before the root candidate projection. A discipline's `applies-when:` controls application, not loading. Apply `cumaru-first` as the priority gate whenever repository work could use Cumaru navigation, workflows, semantic tags, coverage, health checks, lifecycle, update, migration, or guarded `.cumaru/` operations; it chooses an existing surface and creates no second loading mechanism.
