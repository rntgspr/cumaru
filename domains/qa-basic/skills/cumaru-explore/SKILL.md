...
1. **Frame the experiment.** Confirm the hypothesis, affected test levels, and
   the experiment that would resolve the uncertainty.
2. **Agree on a slug** — short, kebab-case (`checkout-flaky-network`, `a11y-keyboard-nav`). Confirm.
3. `cumaru fs exploring/<slug> create` → `cumaru fs exploring/<slug>/index.md create`
4. Open `templates/exploration.md`; frontmatter: `status: idea`, `summary` (8-12 words), `targets:` (levels concerned).
5. Body (free prose): `## Charter` (mission — area, time box, perspective), `## Context`, `## Heuristics / areas to probe`, `## Findings` (running notes), `## Promotion / drop criteria`. Do not add acceptance criteria, scope, or a DAG; that structure means the charter is ready to become a campaign.
6. Run `cumaru tree exploring --rows` and verify the candidate appears.
7. `cumaru doctor`.

## Recipe: run the charter (capture findings)

As the session runs, record observations under `## Findings`. Each is one of:
- **bug** — log it in the tracker, then use `cumaru-intake` to mirror it (a regression test will lock it in later).
...
1. **Decide the destination key.** Tracker item exists → `plans/<KEY>/` (use `cumaru-intake` first if needed). Else → `plans/maintenance-<slug>/`.
...
4. **Remove or carry the charter:** `cumaru fs exploring/<slug> remove`, or `cumaru fs exploring/<slug>/index.md copy plans/<PLAN-ID>/exploration.md` first if the notes are worth keeping.
5. Run `cumaru tree exploring --rows`, `cumaru tree plans --rows`, and `cumaru doctor`.

## Recipe: drop a charter

1. Confirm — "drop is permanent (no absorption; only closed campaigns flow to absorption). Sure?"
2. `cumaru fs exploring/<slug> remove` → run `cumaru tree exploring --rows` → `cumaru doctor`.

**Don't retain charters during close-out.** Only a confirmed consumed charter
is removed after its campaign is absorbed.

## What this skill does NOT do

- **Campaign authoring** — `cumaru-plan`. **Coverage** — explorations never touch `coverage/`; only closed campaigns do, via `cumaru-absorb`. **Tracker mirror** — `cumaru-intake` first if a finding becomes a tracked bug.

## Patterns

| User says | You do |
|---|---|
| "Let's charter a session on checkout under flaky network" | Bootstrap recipe → propose slug → confirm → create from template → run `cumaru tree` |
