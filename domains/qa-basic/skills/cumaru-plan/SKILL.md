...
The campaign-lifecycle skill. From bootstrap of a new `plans/<PLAN-ID>/` to readiness-for-absorption. Absorption itself lives in [`cumaru-absorb`](../cumaru-absorb/SKILL.md).

## Invocation

Use invocation arguments as a campaign ID or requested action. Run `cumaru
doctor --quiet` first; if errors affect `plans/`, `intake/`, `coverage/`, or
`standards/`, surface them and ask whether to repair them before proceeding. If
a tracker key has no intake entry, offer `cumaru-intake`, then resume after it
exists. A campaign is ready for absorption only when every case is green and the
delta draft exists.
...
5. Open `templates/plan.md`; author frontmatter (`key`, `type`, `scope`, `status: in-progress`, `summary`, `targets` = target levels).
...
3. Open `templates/task.md`; frontmatter: `plan`, `task: T<N>`, `depends-on:` (authoring order within the campaign), `concerns:` (`coverage/` paths it touches), `files:` (predicted spec/fixture files), `status: pending`, `targets:` (the levels this case is written at).
...
- Return the case status and handoff evidence to Lead. Dev does not edit the
  plan index, DAG row, or campaign status; Lead reconciles them. Run `cumaru doctor`.
...
4. **Stop.** Do not edit `coverage/` directly — the Lead validates + finalizes during `cumaru-absorb`.

## Recipe: ready for absorption

Verify `delta-draft.md` exists (`status: draft`), every case done + handoff present, then **hand off to [`cumaru-absorb`](../cumaru-absorb/SKILL.md)**. This skill does not perform the absorption.

## What this skill does NOT do

- **Absorption** — `cumaru-absorb`. **Coverage authoring** — `cumaru-coverage` (creates the `scope:` areas). **Intake / explore** — `cumaru-intake`, `cumaru-explore`. **Conventions** — `standards/` (referenced, never restated).
