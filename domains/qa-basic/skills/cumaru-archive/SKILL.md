...
Before Phase 0, summarize the campaign's `summary:`, `scope:`, completed case
count, test levels involved, and proposed sequence. Ask for `walk` to confirm
...
3. **Adjudicate every durable claim** in `delta-draft.md` against the enumerated
   ...
   Require `### Added / Modified / Removed Scenarios` and
   `### Levels / Gaps changed` for every affected area. Reference `standards/`
   instead of restating conventions in coverage bodies.
