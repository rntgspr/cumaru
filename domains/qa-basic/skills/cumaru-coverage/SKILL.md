...
**Contract:**
- **Living state**: every body reflects the coverage as it is now. Closed-work history lives in Git; transient absorption staging is removed.
- **`depends-on:` is the test-prerequisite order** — the areas whose setup/state this one presumes (auth before checkout). It is both the strongest load signal AND the prerequisite sequence. `relates:` is soft ("consider"). It may link a live `intake/<KEY>` during active work, but that local edge must be removed or replaced before its intake host is cleaned.
...
- **Lead-only authoring**: the Dev never writes inside `coverage/` directly. Absorption happens during the Lead's absorption flow (`cumaru-absorb`), driven by the Dev's `delta-draft.md`.
...
2. **Confirm with the user** before creating: name, summary, `depends-on:` (areas whose setup this presumes), `relates:` (the `intake/<KEY>`s it covers), `targets:` (the levels it is exercised at, from `meta.targets.values`).
3. `cumaru fs coverage/<area> create`
4. `cumaru fs coverage/<area>/index.md create`
5. Open `templates/coverage.md`; author the frontmatter (`name`, `summary`, `depends-on` = test-prerequisite areas, `relates`, `targets`).
6. Body — follow the template:
   ...
   - `## Scenarios (GWT)` — observable behaviours as `GIVEN <precondition> WHEN <action> THEN <outcome>`. Each states the acceptance fact it verifies; a live intake relation may assist active traceability, while `## Decisions` retains the stable upstream tracker reference after cleanup. Light pass produces broad scenarios; the deep pass refines.
...
4. **Split into a case file** when a scenario deserves its own file (`cumaru fs coverage/<area>/<case>.md create`; copy the frontmatter shape; move the content; link it under `## Files`).
5. **Promote a case to a subarea** when a flow grows its own internal cases (`cumaru fs coverage/<area>/<subarea> create` + its `index.md`; move content; spawn child cases). Subareas follow the same shape recursively.
...
to the removed local host remains.

## What this skill does NOT do

- **Delta absorption** — `cumaru-absorb` (merges a closed campaign's delta into existing areas).
