...
Load `.cumaru/roles/lead.md`, then `plans/index.md`, `specs/index.md`, and
`exploring/index.md`. Use invocation arguments to select the matching recipe
and confirm each judgment before writing. This domain has one Lead role; do not
invent or refer to an Admin role.
...
**Role**: Lead — the domain's unrestricted single role. Authors everything,
writes handoffs, and drafts deltas.

## Recipe: bootstrap a new plan (tracker-backed)

When the user says "start a plan for AAA-1234" / "vamos começar AAA-1234":

1. **Decide scope by traversing `specs/` under the loading rule.** Read `specs/index.md`, run `cumaru tree specs --rows`, prune candidates by relevance, then recurse with `cumaru tree specs/<area> --deep --rows` only into the areas that survived — never `--deep` on the whole pillar. Terminate at the relevant `<concern>.md` leaves. The proposed `scope:` is exactly the set of leaf paths surfaced by that traversal. **Confirm with the user.**
2. `cumaru fs plans/<KEY> create`
3. `cumaru fs plans/<KEY>/index.md create`
