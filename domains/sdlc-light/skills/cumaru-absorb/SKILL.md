...
version: 1
name: cumaru-absorb
description: Use this skill whenever the user wants to close a plan — verify completion, absorb its delta directly into specs, establish recovery, and clean up the plan only after every gate passes. It is sdlc-light-only and knows the `plans/` and `specs/` lifecycle.
...
- Verify every acceptance criterion in the plan against the implementation,
...
2. **For each mapped claim from Phase 0:**
   ...
   - Identify any originating `exploring/` entry actually consumed by the plan.
...
direct absorption, so no archive staging is introduced.
