...
Load `.cumaru/roles/lead.md` and `exploring/index.md` before selecting a recipe.
Use invocation arguments as the slug or action and confirm each judgment before
writing.
...
4. Frontmatter: `status: idea`, `summary: <one-line>`, `targets: [...]`.
...
4. **Remove or carry over the exploration:**
   - If no longer useful → `cumaru fs exploring/<slug> remove`.
   - If worth preserving → `cumaru fs exploring/<slug>/index.md copy plans/<PLAN-ID>/exploration.md` first, then remove original.
