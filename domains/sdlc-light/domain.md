...
├── plans/        ← tracker-backed and maintenance work while active
...
- **`plans/` — active work items.** One `plans/<PLAN-ID>/` per tracker-backed
  or maintenance item. For tracker-backed items the `key:` field records the
  tracker id; overview and acceptance criteria still live in the plan. Tasks,
  handoffs, evidence, and the delta draft remain inside until guarded direct
  absorption removes the complete plan. There is no intake or archive pillar.
...
close, the `cumaru-absorb` skill reads the plan, updates exact spec owners, and
removes the plan. An originating exploration is removed only when actually
consumed and not shared with active work; unresolved provenance keeps it in
place. Only `specs/` remains as durable system record.
