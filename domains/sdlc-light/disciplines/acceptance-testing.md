...
applies-when: a plan is implemented and about to close; verifying its acceptance criteria with evidence before direct absorption
strictness: 10/10
source:
...
failing criterion is a blocker, not a review comment — never close or absorb with one in FAIL.

## Cycle

1. **Source the criteria** — the EARS / RFC 2119 criteria already authored in
   `plans/<PLAN-ID>/index.md`, plus any `## Requirements (EARS / RFC 2119)` in
   the spec areas the plan's `scope:` touches. This applies equally to
   tracker-backed and maintenance plans. If a behavior has no criterion, add it
   to the plan first — don't invent a pass.
...
3. **Act on the result:**
   - All PASS → proceed to direct absorption into `specs/`.
...
- Closing / absorbing while any criterion shows FAIL.
