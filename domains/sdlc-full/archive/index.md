...
- **Archive is transient.** Directories exist only while absorption is in flight. After specs are updated the durable record **is** `specs/`; nothing is carried forward.
...
- Looking for durable history — read the relevant `specs/` area, and `git log` to see which plans built it.
