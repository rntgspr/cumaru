...
- Treat authorization for the requested scope as durable. Do not ask again for
  routine metadata or implementation judgments grounded in project evidence.
- Stop when user input or new authority is genuinely required; do not guess at
  unresolved scope or ownership, destructive cleanup, or decisions that
  materially change intent.
