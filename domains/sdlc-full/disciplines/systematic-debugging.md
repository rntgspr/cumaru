...
1. **Root cause** — reproduce reliably when practical, then trace backward from
   the symptom to its origin. When immediate reproduction is unavailable, use
   attributable logs, traces, reports, or captured state to form a labeled
   hypothesis; do not present it as a confirmed cause.
...
**Three-fix rule:** after three failed fixes, STOP changing code and reassess
the evidence, assumptions, reproduction, and competing hypotheses before a
fourth attempt. Consider an architectural defect only when observed evidence
supports it; the attempt count alone proves nothing about the design.
