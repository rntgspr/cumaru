...
A pillar for **active execution plans** — one directory per item or internal initiative currently being worked on. Authored by the Lead at planning time; absorbed directly into the durable pillar on close.
...
- **Plan body for tracker-backed plans carries `## Plan / DAG`, `## Out of scope`, `## Risks` only.** Overview and Acceptance Criteria live in the linked `intake/<KEY>.md` and are not duplicated here. Slug-based plans keep both inside the plan body.
- **Stories are linear at the Lead level.** Only one plan from the same story is active at a time. Cross-ticket coordination happens in the story's `## Coordination` section in `intake/<STORY-KEY>.md`.
...
- Closing a plan → Dev writes `delta-draft.md`; Lead validates, finalizes it in place, absorbs into specs.

## When NOT to use

- Item scope, Overview, or Acceptance Criteria → `intake/<KEY>.md`.
- Description of the system as it is today → `specs/<area>/`.
- Pre-plan ideation, sketches, options analysis → `exploring/<slug>/`.
- A locally authored issue before its plan exists → `issues/<slug>/`.
- Completed work → `plans/<PLAN-ID>/`.
