...
`archive/<PLAN-ID>/`, `exploring/<slug>/`, and `issues/<slug>/` are not drilled by default — opening them is opt-in (user reference, or the active plan's `scope:` pointing there). When in doubt, ask before loading more.
