...
- **Authoring is the Lead's.** Dev never writes inside `specs/` directly — spec absorption happens during the Lead's absorption flow, driven by the Dev's `delta-draft.md`.
...
- Closed-plan staging details → `plans/<PLAN-ID>/` only while absorption is in flight.
