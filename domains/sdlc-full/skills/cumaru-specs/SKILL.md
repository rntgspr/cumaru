...
The living-spec skill. Three recipes that grow the `specs/` tree across the project's lifetime: **bootstrap** (initial scaffold), **deepen** (light → deep pass), **consolidate** (rewrite a changelog-shaped body as current state).
...
**Contract:**
- **Living state**: every body reflects the system as it is now. There is no second store of absorbed work — this pillar *is* the record, and `git log` is the only cross-reference back to the plans that built it.
- **Bootstrap on demand**: an area is created the first time a plan declares it in `scope:` — don't seed empty areas in advance.
- **Lead-only authoring**: Dev never writes inside `specs/` directly. Spec absorption happens during the Lead's absorption flow (see `cumaru-absorb`), driven by Dev's `delta-draft.md`.

## Recipe: bootstrap a spec area

When a new area `<area>` is within the authorized task (typically during initial install, or the first time a plan touches a yet-undocumented surface):
...
2. Derive name, summary, `depends-on:` (other areas whose contract you actually
   need to read alongside), and `targets:` (component keys from
   `meta.targets.values`) from project evidence. Stop only when scope,
   ownership, or the area boundary remains unresolved.
...
**What NOT to do:**
- Don't create areas outside the authorized scope or across an unresolved ownership boundary — a bad split poisons every later plan.
...
4. **Split into a concern file** when a topic is large enough to deserve its own file:
   - `cumaru fs specs/<area>/<concern>.md create`
   - Copy the frontmatter shape from `templates/spec.md`. Set `name: <concern>`, repeat `targets:`, give it its own `summary:`.
...
5. **Promote a concern to a subarea** when it has grown beyond a flat file and has its own internal concerns:
   - `cumaru fs specs/<area>/<subarea> create`
   - `cumaru fs specs/<area>/<subarea>/index.md create`
...
read: the signal is the body itself reading like a changelog — accreted
"then we added…" passages — instead of a flat statement of what is true now.
...
3. **Rewrite the area's body into a single coherent spec.** Integrate every past change as if it had always been part of the system. Where two contradict, reflect the **current** state. Modified requirements appear in their modified form; removed requirements are gone.
4. run `cumaru tree specs --rows` and confirm the area's `summary:` still describes it.
5. `cumaru doctor`.

**What NOT to do:**
- Don't consolidate halfway ("kept some for clarity" violates the model). Either consolidate or don't.
- Don't consolidate on every plan close — pay the cost only when the body genuinely reads as history rather than state.

## Spec absorption during absorption (NOT this skill)

When a plan closes via `cumaru-absorb`, the Lead's absorption flow opens each `specs/<area>` in the plan's `scope:` and:
...
4. Commits with a message naming every KEY absorbed — the only cross-reference back to the plan.

This skill provides the recipes to **grow** the spec tree (bootstrap, deepen, consolidate). The `cumaru-absorb` skill provides the recipe to **absorb** a closed plan's delta into already-existing areas. Both write to `specs/` — but only the Lead, never the Dev.

## What this skill does NOT do

- **Delta absorption** — `cumaru-absorb`. This skill grows specs from first principles or refactors them; absorption merges plan deltas into them.
...
| User says | You do |
|---|---|
| "Bootstrap the specs" / "scaffold the spec areas" | Bootstrap recipe → derive area list and routine metadata from instructions, README, and code → stop only for unresolved scope or ownership → create each area |
...
| "Consolidate specs/payments" / "compactar a área de payments" | Consolidate recipe → read the body → recover history from git if needed → rewrite as current state |
| "Add a new spec area for telemetry" | Bootstrap recipe with `<area>=telemetry` |

Use `cumaru tree specs` for navigation; pair with `cumaru-plan` (which declares `scope:` paths), `cumaru-absorb` (which absorbs deltas), and `cumaru-doctor` to verify post-op.
