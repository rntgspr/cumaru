...
Use invocation arguments as the issue slug or requested action. If the request
is tracker-backed, hand off to `cumaru-intake` instead. Before creating,
refining, promoting, or dropping an issue, confirm the slug, type, priority,
applications, and the requested lifecycle decision.

## Layout

```
issues/<slug>/
└── index.md          <- [type!, priority!, status!, summary!, targets!]
```
...
2. `cumaru fs issues/<slug> create`
3. `cumaru fs issues/<slug>/index.md create`
4. Open `templates/issue.md` and author the issue from the chat and repository evidence:
   - Set `type`, `priority`, `status: open`, `targets`, and a stable `summary:`.
...
4. Remove the issue only after the plan is valid: `cumaru fs issues/<slug> remove`.
...
2. `cumaru fs issues/<slug> remove`
