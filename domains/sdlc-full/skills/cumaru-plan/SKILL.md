...
The plan-lifecycle skill. Covers everything from bootstrap of a new `plans/<PLAN-ID>/` to readiness-for-absorption. Absorption itself lives in [`cumaru-absorb`](../cumaru-absorb/SKILL.md).

## Invocation

Use invocation arguments as a plan ID or requested action. Run `cumaru doctor
--quiet` first; if errors affect `plans/` or `intake/`, surface them and ask
whether to repair them before proceeding. For an unknown tracker key with no
`intake/<KEY>.md`, offer `cumaru-intake`, then resume after intake exists. For
an existing plan, ask whether to add a task, write a Dev handoff, draft the
delta, or prepare for absorption. Confirm every scope, task-breakdown, and handoff
...
1. **Pre-check.** `intake/<KEY>.md` must exist. If absent, hand off to `cumaru-intake` first.
...
4. `cumaru fs plans/<KEY> create`
5. `cumaru fs plans/<KEY>/index.md create`
...
2. `cumaru fs plans/maintenance-<slug> create`
3. `cumaru fs plans/maintenance-<slug>/index.md create`
4. Frontmatter — no `key:`, no `type:`, no `epic:`/`story:`. Required: `scope`, `status: in-progress`, `summary`, `targets`.
...
3. Open `templates/task.md`; author the frontmatter:
   ...
   - `status: pending`.
   - `targets:` from the task's actual touch surface (may be a subset of the plan's `targets:`).
...
1. `cumaru fs plans/<PLAN-ID>/handoff-t<N>.md create`
...
5. Return the task status and handoff evidence to Lead. Dev does not edit the
   plan index, DAG row, or plan-level status; Lead reconciles them.
6. `cumaru doctor` — file refs check should find every link in the handoff's `<!-- cumaru:touched -->` table resolves on disk.
...
2. `cumaru fs plans/<PLAN-ID>/delta-draft.md create`
...
5. **Stop here.** Do not edit `specs/` directly. The Lead validates the draft and finalizes during `cumaru-absorb`.
6. **Hand off to the Lead** — the next step is `cumaru-absorb` (close + absorb).

## Recipe: transition a plan to "ready for absorption"

When the user says "this plan is done" / "let's absorption it":
...
3. **Hand off to [`cumaru-absorb`](../cumaru-absorb/SKILL.md)** — it runs the absorption recipe (move to `plans/<PLAN-ID>/`, finalize delta, absorb into specs, and remove plan files).

This skill does NOT perform the absorption itself — that's `cumaru-absorb`'s job. This skill just verifies the plan is in a state where absorption can run.
...
| "Write the handoff for T2 of AAA-1234" | Handoff recipe (Dev) → write handoff-t2.md → flip t2.md status → return evidence for Lead DAG reconciliation |
