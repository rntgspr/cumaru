...
version: 1
name: cumaru-absorb
description: Use this skill whenever the user wants to close a plan — verify completion, absorb its delta directly into specs, establish recovery, and clean up the plan only after every gate passes. It is sdlc-full-only and knows the `intake/`, `issues/`, `plans/`, and `specs/` lifecycle.
summary: Verify a plan is complete, absorb its spec delta, establish recovery, and remove it only after every direct close-out gate passes.
---
...
- Resolve the acceptance source: tracker-backed criteria live in
  `intake/<KEY>.md`; maintenance criteria live in the plan. Verify every
  acceptance criterion against the implementation,
...
- If implementation is incomplete, infeasible, blocked, partially delivered,
  ...
  plan, tasks, handoffs, delta draft, and auxiliary evidence intact. Do not
  enter the absorption phases. Discarding the plan requires a separate explicit
...
- The `git` skill is installed. A committed recovery point is required before
  direct-absorption cleanup. If absent, refuse and keep the plan open.
- Inventory every existing file under `plans/<PLAN-ID>/`, including ignored
  ...
  be empty before editing `specs/`. A status check without ignored files can
  miss evidence.
- Git writes require explicit user authorization. Without the evidence baseline
  or authorization for a required commit, keep the plan open and stop before
...
1. **Read and validate the delta-draft.** Open `plans/<PLAN-ID>/delta-draft.md`. Verify:
   ...
   - Finalize the delta in place before durable edits: remove `status: draft`,
     tighten its claims, and keep it under the plan until guarded cleanup.
2. **For each mapped claim from Phase 0:**
   - Edit its exact `specs/<area>/<owner>.md` file. Use the area `index.md` only
   ...
   - Identify any originating `intake/`, `issues/`, or `exploring/` entry
     actually consumed by the plan.
     A shared source, active consumer, or unresolved provenance link blocks its
     removal.
3. **Handle ghost deltas** (delta says "no spec change required") only when the
   implementation and every acceptance criterion are complete and verified,
   and the durable spec genuinely remains unchanged. Infeasibility, partial
...
The plan remains intact throughout absorption and validation.

## Phase 2 — validate the durable result

1. Inspect the affected specs and reconcile every acceptance criterion with its
   implementation evidence and durable claim or explicit no-spec-change
...
2. Run `cumaru doctor`. Any failed check, unresolved claim, missing evidence,
   or changed cleanup target stops close-out with the complete plan intact.
...
```bash
git add -- .cumaru/specs/<verified-file> .cumaru/plans/<PLAN-ID>/
...
For a verified no-spec-change result, omit the durable-file argument. A current
clean recovery commit that already contains the complete plan and names its key
may be reused; do not create an empty commit merely to advance the recipe.

## Phase 4 — clean transient work

Only after Phase 3 succeeds, remove the plan and each confirmed consumed
...
Run `cumaru tree plans --rows`, `cumaru tree specs --rows`, and
...
Phase 1 updates the living spec while leaving the plan intact. Phase 2 proves
acceptance and validates the durable result. Phase 3 commits that result together
with every cleanup target. Only Phase 4 removes the plan. This domain uses
...
| User says | You do |
|---|---|
| "Absorb AAA-1234" / "close plan X" / "finalize plan X" | Pass the completion and evidence gate, then run Phase 0 through Phase 4; preserve the plan on any blocker |
