...
3. Set the template-required frontmatter. Preserve an existing refined item's `type`, `targets`, `relates`, `summary`, provenance, and body; refresh source-derived status only when the source was actually read.
4. Stage unedited source prose in a `<!-- BEGIN RAW (tracker: <name>) ... END RAW -->` block while refining. Record `key`, scalar `tracker`, `type`, `status`, `synced-at`, `targets`, and `relates` from evidence.
...
6. Set `targets` from `config.yaml > meta.targets.values`, verify parent and cross-item `relates`, set a valid `summary`, then remove the complete RAW block.
