...
Configuration reconciliation is read-only. General update transactions keep
private recovery snapshots only for their managed surfaces; they are removed
...
| [`src/cmd_update.sh`](../../src/cmd_update.sh) | Config drift reporting and update transaction handling. |
...
| `_update_transaction_apply` | Applies staged framework and agent artifacts with temporary recovery. |
...
| [`tests/spec/update/transaction_spec.sh`](../../tests/spec/update/transaction_spec.sh) | Config publication and rollback boundaries. |
