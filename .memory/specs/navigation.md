...
1. Normalize a Markdown target to its parent directory.
2. Shallow mode lists direct Markdown files except `index.md` and indexed child
   directories. Deep mode recursively validates all non-hidden descendants.
3. Read only YAML frontmatter to obtain summaries.
4. Emit valid candidates after C-locale sorting; aggregate deep-mode defects
   and return nonzero after the walk.
