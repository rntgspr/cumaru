...
3. Bootstrap order is kernel, domain, every installed discipline, then the
   root `cumaru tree .` projection where a SessionStart hook is supported.
...
3. Run staged `cumaru doctor --quiet`, then publish all managed surfaces or
...
| Staged doctor rejection | `1` | none live |
| Handled publication failure | `1` | managed surfaces restored |

## Transaction and recovery

Agent apply uses the project-local update transaction: same-filesystem staging,
`.cumaru-update.lock`, exact managed-surface snapshots, staged doctor, and
logical multi-path publication. A handled failure restores `.cumaru/` and all
adapter surfaces; this is not one filesystem-wide atomic rename.
...
| [`../../src/cmd_update.sh`](../../src/cmd_update.sh) | Dry-run, staged adapter switch, publication, and rollback. |
...
| [`../../tests/spec/update/transaction_spec.sh`](../../tests/spec/update/transaction_spec.sh) | Staged validation and full-surface rollback. |
