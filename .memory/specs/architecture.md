...
| Update staging or validation failure | `1` | none to live surfaces |
| Handled publication failure | `1` | managed surfaces restored |

## Transaction and recovery

Update apply uses same-project staging, `.cumaru-update.lock`, exact managed
surface snapshots, staged doctor validation, and handled rollback. This is a
logical multi-path transaction, not one filesystem-wide atomic rename.
...
| `src/cmd_update.sh` | Ownership-aware planning and transactional publication. |
...
| `_update_transaction_apply` | Stage, doctor-check, publish, and restore managed surfaces. |
...
| `tests/spec/update/*.sh` | Ownership, version gates, dry-run, transaction, rollback, tags. |
