...
Update preservation is name-based. Schema host and body-type declarations tell
the CLI how to validate or interpret a tag, but do not permit update to discard
...
`--rows` parses only `default` `[Link, Description]` rows and resolves links
