...
Every mutating `--apply` first verifies that the project is inside a Git work
tree and that `git status --porcelain --untracked-files=all` is empty. Staged,
unstaged, and untracked changes all block mutation before Cumaru writes managed
files. The repository must have a commit, and `.cumaru/config.yaml` plus
`.cumaru/index.md` must be tracked. Commit, stash, or clean the work tree first;
Git history is the recovery boundary.
