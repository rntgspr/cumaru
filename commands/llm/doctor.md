...
1. **Synthesize a one-paragraph summary for the user** — totals, then list errors first and warnings second, grouped by check (schema conformance / orphan rows & files / stale work-marker files / file refs / external tools). Mention any `→` hints verbatim, since each is a known remediation path.
...
- **File references not found** → For each `<file>: <path>` in the `→` detail, ask whether the path is wrong (edit the path-list block via `llm tag set`) or the file was lost (recreate or remove the reference).
