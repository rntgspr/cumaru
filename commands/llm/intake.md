...
9. **Close out.** Print the final path and run `llm doctor` against the same `DOT_LLM_DIR`. Doctor's orphan check should be clean — the new file is claimed by the row you just wrote. Report: refinement complete, EARS warnings (none expected), `apps:` value applied (or the blocker noted in step 6), and doctor totals.
