...
def validate_meta($meta):
  if ($meta | type) != "object" then err(["meta"]; "must be an object")
    ...
    unknown($meta; ["targets", "tags", "specification_dir", "coverage", "compatibility"]; ["meta"]),
    (if ($meta.targets? | type) != "object" or (strings($meta.targets.values?) | not) or ($meta.targets.values | length) == 0 or (($meta.targets.values | unique | length) != ($meta.targets.values | length))
      then err(["meta", "targets", "values"]; "must be a non-empty array of unique strings") else unknown($meta.targets; ["values"]; ["meta", "targets"]) end),
    (if $meta.tags? == null then empty
      elif ($meta.tags | type) != "object" then err(["meta", "tags"]; "must be an object")
    ...
    (if $meta.compatibility? == null then empty
      elif ($meta.compatibility | type) != "object" then err(["meta", "compatibility"]; "must be an object")
      else
