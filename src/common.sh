...
This project uses the \`.cumaru/\` framework — a spec-driven, agent-friendly knowledge structure. At the start of every session in this repository, enter the framework in this order: read \`$rel_index\` (the kernel), \`$rel_domain\` (the domain), and every installed file under \`.cumaru/disciplines/\` (mandatory execution disciplines), then run \`cumaru tree .\` to project the root's current candidates and their summaries. Discipline \`applies-when\` fields control application, never loading. Prune tree candidates by relevance — never prune the execution disciplines.
