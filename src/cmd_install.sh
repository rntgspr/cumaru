...
cmd_install() {
  ...
  if [[ -e "$target" ]]; then
    red "✗ target $target is already installed"
    yellow "→ Use 'cumaru update' for framework content or 'cumaru update skills <agent> --with <skill>' for opt-in skills."
    yellow "→ To replace the domain, run 'cumaru uninstall' before installing again."
    return 1
  fi
  ...
  local schema_domain="$domain"
  schema_validate_domain "$domain_src" "$schema_domain" || return $?

  # Pre-validate skills before any write — fail fast.
  _framework_validate_opt_in_skills "$SKILLS_SRC" "${with_skills[@]+"${with_skills[@]}"}" || return 1
...
Next steps:
  1. Read $target/domain.md for the selected domain's target vocabulary and
     post-install setup contract.
  2. Edit $target/config.yaml — under meta.targets.values, declare only values
     allowed by that domain contract.
  3. Run health checks:
       cumaru doctor

Cumaru artifacts were installed for '$agent'. Adapter selection is stateless;
manage each adapter explicitly with cumaru update skills|commands <agent>.
EOF
}
...
# Explicit opt-ins replace only the requested opt-in directories.
...
_framework_install_skills() {
  ...
  [[ -n "$agent" ]] || { red "✗ adapter target is required"; return 2; }
  local skills_dir
  ...
  _framework_install_opt_in_skills "$parent" "$SKILLS_SRC" "$agent" "${with_skills[@]+"${with_skills[@]}"}"
}

# Validate every requested opt-in skill before a caller mutates project state.
# Args: skills source root, skill names.
_framework_validate_opt_in_skills() {
...
# Install only explicitly requested opt-in skills into one adapter directory.
# Args: project parent, skills source root, adapter, skill names.
_framework_install_opt_in_skills() {
  local parent="$1" skills_src="$2" agent="$3"; shift 3
...
_framework_deprecated_commands() {
  ...
  [[ -n "$cmds_dir" ]] || return 0
  local cmds_src="$framework_src/commands"
  [[ -d "$cmds_dir/cumaru" ]] || return 0
  [[ -d "$cmds_src" ]] || return 0
  local file rel
  while IFS= read -r -d '' file; do
    rel="${file#"$cmds_dir"/}"
    [[ -f "$cmds_src/$rel" ]] || echo "$rel"
  done < <(find "$cmds_dir/cumaru" -type f -name '*.md' -print0)
}

# Remove Cumaru command launchers that no longer exist in the domain source.
# Commands outside the owned cumaru namespace remain untouched.
# Args: parent framework_src
_framework_prune_deprecated_cumaru_commands() {
  local parent="$1" framework_src="$2"
  local agent="${CUMARU_AGENT_OVERRIDE:-}"
  [[ -n "$agent" ]] || { red "✗ adapter target is required"; return 2; }
  local cmds_dir
  cmds_dir=$(_agent_commands_dir "$parent" "$agent")
  [[ -n "$cmds_dir" && -d "$cmds_dir/cumaru" ]] || return 0
  ...
  while IFS= read -r rel; do
    ...
    file="$cmds_dir/$rel"
    rm -f "$file" || return 1
    yellow "  - removed deprecated: $(_agent_rel_path "$parent" "$file")"
  done < <(_framework_deprecated_commands "$parent" "$framework_src")
}
...
EOF
  ...
  --with <skill>         include an opt-in skill (looked up in skills/<skill>/SKILL.md
                         in the Cumaru checkout). Repeatable.
...
Without `agent`, Cumaru installs the generic `.agents/` integration. A selected
agent uses its best native project surfaces, without changing config.yaml.

Install is only for initial project adoption and refuses an existing `.cumaru/`.
Use `cumaru update` for framework refreshes. Add opt-in skills to an existing
project with `cumaru update skills <agent> --with <skill> [--apply]`.
...
Examples:
  ...
  cumaru update skills codex --with git                # preview an existing install
  cumaru update skills codex --with git --apply        # add the opt-in
EOF
}
