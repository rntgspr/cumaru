...
Model:
  ...
  array in config.yaml. .cumaru/ and agent adapter files are always excluded.
...
_coverage_spec_dir() {
  ...
  declared=$(yq -r '.meta.specification_dir // ""' "$CONFIG")
  if config_tree_is_v9 "$CONFIG" && [[ -n "$declared" ]]; then
    resolved=$(config_tree_root_directory "$declared") || return 1
    printf '%s\n' "$resolved"
    return 0
  fi
  printf '%s\n' "$declared"
...
_coverage_source_globs() {
  [[ -f "$CONFIG" ]] || return 0
  yq '.meta.coverage.source[]' "$CONFIG"
}
...
cmd_coverage() {
  ...
  [[ -d "$CUMARU_DIR" ]] || { red "✗ $CUMARU_DIR not found — run 'cumaru install' first" >&2; return 1; }
  [[ -f "$CONFIG" ]] || { red "✗ $CONFIG not found — not a Cumaru tree?" >&2; return 1; }

  local spec_dir
  spec_dir=$(_coverage_spec_dir)
  [[ -n "$spec_dir" ]] || spec_dir="specs"
  if [[ ! -d "$CUMARU_DIR/$spec_dir" ]]; then
    red "✗ $CUMARU_DIR/$spec_dir/ not found" >&2
    yellow "  → set 'specification_dir:' under 'meta:' in $CONFIG to the pillar that holds the durable specification (default: specs)" >&2
    return 1
  fi
  ...
  if [[ -z "$proj" ]] || ! git -C "$proj" rev-parse --is-inside-work-tree >/dev/null 2>&1; then
    red "✗ cumaru coverage needs a git work tree — the source file list comes from 'git ls-files'" >&2
    return 1
  fi
