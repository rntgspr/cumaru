...
#
# Removes only what `cumaru install` creates across every supported adapter:
...
cmd_uninstall() {
  ...
  if [[ $has_target -eq 1 ]]; then
    ...
    if [[ -z "$target_abs" || "$target_abs" == "/" || "$target_abs" == "$HOME" || \
          ! -f "$target/index.md" || ! -f "$target/config.yaml" ]]; then
      red "✗ refusing to uninstall — $target is not a cumaru install (expected index.md + config.yaml)"
      return 1
    fi
  fi

  local has_artifacts=0
  local agent skills_dir commands_dir instructions
  for agent in generic claude codex opencode; do
    ...
    [[ -n "$instructions" && -f "$instructions" ]] && grep -qE "BEGIN (CUMARU|DOT-LLM)-HOOK" "$instructions" && has_artifacts=1
    [[ "$agent" == opencode && -f "$parent/opencode.json" ]] && jq -e '.instructions | index(".cumaru/index.md") != null' "$parent/opencode.json" >/dev/null 2>&1 && has_artifacts=1
  done

  if [[ $has_target -eq 0 && $has_artifacts -eq 0 ]]; then
    say "Nothing to uninstall — no .cumaru tree or Cumaru adapter footprint at $parent."
    return 0
  fi
  ...
  echo "  - Cumaru-owned artifacts for every supported adapter"
  ...
  # --- act ---
  for agent in generic claude codex opencode; do
    _agent_remove_adapter "$parent" "$agent" "" || { red "✗ failed to remove $agent artifacts; .cumaru was preserved"; return 1; }
  done

  # Generic, Codex, and OpenCode share this path. Scoped adapter removal must
  # preserve it, so global uninstall owns the final shared-skill cleanup.
  _agent_remove_skills_at "$parent/.agents/skills" || {
    red "✗ failed to remove shared Cumaru skills; .cumaru was preserved"
    return 1
  }
...
What it removes (only what `cumaru install` created):
  1. The .cumaru/ tree.
  2. Cumaru-owned files from every supported adapter.
...
Examples:
  cumaru uninstall                     # remove ./.cumaru + every Cumaru adapter footprint
