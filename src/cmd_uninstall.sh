...
cmd_uninstall() {
  ...
  if [[ $has_target -eq 1 ]]; then
    # Defense-in-depth before a recursive delete: a pathological argument like
    # `llm uninstall / --yes` or `llm uninstall "$HOME" --yes` must never reach
    # rm -rf. Canonicalize, then refuse roots/home and anything not named .llm
    # (what install creates). Delete the resolved path, not the raw arg.
    local target_abs
    target_abs="$(cd "$target" 2>/dev/null && pwd)" || target_abs=""
    if [[ -z "$target_abs" || "$target_abs" == "/" || "$target_abs" == "$HOME" || "$(basename "$target_abs")" != ".llm" ]]; then
      red "✗ refusing to remove '$target' — not a .llm install directory"
      return 1
    fi
