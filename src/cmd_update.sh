...
#
# Version drift is gated by source and local config versions. Every downgrade is
# refused, and a higher integer boundary is dry-run only.
#
...
# Enforce a recoverable Git baseline when the project already uses Git.
_update_check_git_recovery() {
  local parent="$1" status
  if ! command -v git >/dev/null 2>&1; then
    yellow "⚠ Git is unavailable; continuing without a Git recovery point."
    return 0
  fi
  if ! git -C "$parent" rev-parse --is-inside-work-tree >/dev/null 2>&1; then
    yellow "⚠ Not a Git work tree; continuing without a Git recovery point."
    return 0
  fi
  status=$(git -C "$parent" status --porcelain --untracked-files=all 2>/dev/null) || {
    red "✗ cannot inspect Git work tree status"
    return 1
  }
  if [[ -n "$status" ]]; then
    red "✗ Git work tree has pending changes; refusing to mutate"
    yellow "→ Commit, stash, or clean tracked, staged, unstaged, and untracked changes before running update."
    printf '%s\n' "$status" >&2
    return 1
  fi
  if ! git -C "$parent" rev-parse --verify HEAD >/dev/null 2>&1; then
    red "✗ cumaru update requires a committed Git baseline"
    yellow "→ Commit the installed Cumaru tree before running a mutating update."
    return 1
  fi
  if ! git -C "$parent" ls-files --error-unmatch -- ".cumaru/config.yaml" ".cumaru/index.md" >/dev/null 2>&1; then
    red "✗ .cumaru/config.yaml and .cumaru/index.md must be tracked by Git"
    yellow "→ Commit the installed Cumaru tree so Git can recover an update."
    return 1
  fi
}

_update_validate_artifact_source() {
  local source_domain="$1" domain="$2"
  local source_version target_version
  schema_validate_domain "$source_domain" "$domain" || return $?
  source_version=$(yq -r '.version // ""' "$source_domain/config.yaml" 2>/dev/null || true)
  target_version=$(yq -r '.version // ""' "$CONFIG" 2>/dev/null || true)
  if [[ "$source_version" != "$target_version" ]]; then
    red "✗ agent artifacts must come from the installed framework version"
    return 1
  fi
}

# Report retired v8 archive surfaces without changing adopter-owned state.
# Args: project parent
_update_report_deprecated_archive() {
  local parent="$1" found=0 path

  if [[ -e "$CUMARU_DIR/archive" ]]; then
    yellow "⚠ deprecated archive artifact: .cumaru/archive/"
    found=1
  fi
  if yq -e '.root.archive != null' "$CONFIG" >/dev/null 2>&1; then
    yellow "⚠ deprecated archive configuration: .cumaru/config.yaml root.archive"
    found=1
  fi

  for path in \
    "$parent/.agents/skills/cumaru-archive" \
    "$parent/.claude/skills/cumaru-archive" \
    "$parent/.codex/skills/cumaru-archive" \
    "$parent/.opencode/skills/cumaru-archive" \
    "$parent/.claude/commands/cumaru/archive.md" \
    "$parent/.opencode/commands/cumaru/archive.md"; do
    [[ -e "$path" ]] || continue
    yellow "⚠ deprecated archive artifact: ${path#"$parent/"}"
    found=1
  done

  if [[ $found -eq 1 ]]; then
    yellow "→ These artifacts are no longer used by Cumaru. Inspect and reconcile remaining evidence deliberately; general update will not delete or migrate it."
  fi
}

_update_clear_artifact_kind() {
  local parent="$1" kind="$2" agent="$3" commands_dir command_file
  if [[ "$kind" == skills ]]; then
    _agent_remove_skills_at "$(_agent_skills_dir "$parent" "$agent")"
    return $?
  fi
  commands_dir=$(_agent_commands_dir "$parent" "$agent")
  [[ -n "$commands_dir" && -d "$commands_dir/cumaru" ]] || return 0
  while IFS= read -r -d '' command_file; do
    rm -f "$command_file" || return 1
  done < <(find "$commands_dir/cumaru" -type f -name '*.md' -print0)
}

# Undo every file this apply wrote: remove the ones it created and restore the
# pre-write copy of the ones it replaced. An apply is all-or-nothing, so a write
# failure or a rejected post-update gate must leave no half-updated tree.
# Args: backup_dir, then every path created by this run.
_update_rollback_writes() {
  ...
  while IFS= read -r -d '' file; do
    rel="${file#"$backup_dir"/}"
    cp "$file" "$CUMARU_DIR/$rel" || {
      red "  ✗ rollback failed for $rel — restore the tree from Git"
      return 1
    }
...
_update_render() {
  ...
  if [[ -n "$src_tags$tgt_tags" ]]; then
    echo "Tags (bodies preserved verbatim):"
    while IFS= read -r name; do
      [[ -z "$name" ]] && continue
      if grep -qxF "$name" <<< "$tgt_tags"; then
        if _update_tag_is_empty "$tgt" "$name"; then
          ...
          echo "    [=] $name — body preserved verbatim."
        fi
      else
...
Usage:
  ...
  cumaru update skills [<none|claude|codex|opencode>] [--with <skill>...] [--apply|--clear]
...
Arguments:
  ...
  skills         update ONLY framework skills for installed agent target(s).
                  ...
                  skills wholesale. --clear removes immediately after the Git
                  recovery check. --with installs only the requested opt-in
                  skills. Skips the .cumaru/ file merge and commands.
  commands       update ONLY slash commands for installed agent target(s).
                  Without --apply, prints a dry-run summary. With --apply, replaces
                  commands wholesale. --clear removes immediately after the Git
                  recovery check. Skips the .cumaru/ file merge and skills.
  config         report schema and domain-default drift for agent-led config
                 reconciliation. It never writes config.yaml.
  agent <name>   preview or directly install a native agent integration.
                 With --clear, remove that integration immediately. Omit <name>
                 with --clear to remove every Cumaru agent integration.
...
Options:
  ...
  --apply        rebuild framework Markdown from canonical source while
                   preserving matching tag bodies, or replace explicitly named
                   agent artifacts after the Git recovery check. config.yaml is
                   adopter-owned and must be changed deliberately by an agent.
                  Without it, prints a structured per-file review for the LLM.
  --clear        remove the selected Cumaru artifact surface immediately after
                  the Git recovery check. This is not a preview and does not
                  require --apply. Omit the agent to clear that surface for all
                  supported adapters.
  --with <skill> install one opt-in skill for the explicit adapter. Repeatable;
                  supported only by the skills target and previewed without
                  --apply.

Agent artifacts:
  Skills and supported slash commands are framework-owned artifacts addressed
  by an explicit adapter. --apply installs them. --clear immediately removes
  only Cumaru files and never removes adapter directories.

Transaction safety:
  In a Git work tree, every --apply or --clear mutation requires no staged,
  unstaged, or untracked changes plus a committed, tracked Cumaru baseline.
  Outside Git, Cumaru warns and continues without a Git recovery point.
...
Version drift:
  Local and source config versions are the sole version gate. Versions are integer migration
...
Examples:
  ...
  cumaru update skills codex --with git  preview the Codex git opt-in
  ...
  cumaru update agent claude --clear      immediately remove Claude-owned files
  cumaru update agent --clear             immediately remove every adapter set
...
cmd_update() {
  ...
  local source_version target_schema_version target_version source_schema_domain
  ...
  [[ -n "$target_schema_version" ]] || { red "✗ local config is missing version"; return 1; }
  [[ -n "$source_version" ]] || { red "✗ source config is missing version"; return 1; }
  ...
  say "Target: $CUMARU_DIR (config version ${target_version:-unset})"
  ...
  _update_report_deprecated_archive "$parent"
  ...
  # 5) v9 uses explicit per-entry framework ownership. Older installed trees
  # retain their source traversal during the migration window.
  local rels=() source_rels=() rel source_rel candidate candidate_file
  if config_tree_is_v9 "$CONFIG"; then
    candidate_file=$(mktemp) || return 1
    _update_v9_candidates "$source_domain" "$CUMARU_DIR" > "$candidate_file" || {
      rm -f "$candidate_file"
      return 1
    }
    while IFS=$'\t' read -r rel source_rel; do
      [[ -n "$rel" && -n "$source_rel" ]] || continue
      if [[ -n "$path_filter" ]]; then
        [[ "$rel" == "$path_filter" || "$rel" == "$path_filter"/* ]] || continue
      fi
      rels+=("$rel")
      source_rels+=("$source_rel")
      if [[ "$rel" != "$source_rel" && -e "$CUMARU_DIR/$source_rel" ]]; then
        yellow "⚠ stale framework path retained for review: $source_rel (current destination: $rel)"
      fi
    done < "$candidate_file"
    rm -f "$candidate_file"
  else
  ...
  if [[ $apply -eq 1 ]]; then
    _update_check_git_recovery "$(cd "$parent" && pwd -P)" || return $?

    # The trailing gate can only adjudicate what this run wrote when the tree
    # already passed it, so record that baseline before the first write.
    local gate_was_clean=0
    bash "$SCRIPT_DIR/cumaru" doctor --quiet >/dev/null 2>&1 && gate_was_clean=1

    local backup_dir="" created=()

    # .cumaru/ file merge.
    if [[ $total -gt 0 ]]; then
      backup_dir=$(mktemp -d) || { red "✗ cannot stage an update rollback point"; return 1; }
      for index in "${!changed[@]}"; do
        rel="${changed[$index]}"
        source_rel="${changed_source_rels[$index]}"
        local src="$source_domain/$source_rel" tgt="$CUMARU_DIR/$rel"
        mkdir -p "$(dirname "$tgt")" || {
          red "  ✗ failed to create parent for $rel"
          _update_rollback_writes "$backup_dir" "${created[@]+"${created[@]}"}"
          return 1
        }
        if [[ ! -f "$tgt" ]]; then
          cp "$src" "$tgt" || {
            red "  ✗ failed to create $rel"
            _update_rollback_writes "$backup_dir" "${created[@]+"${created[@]}"}"
            return 1
          }
          created+=("$tgt")
        ...
        mkdir -p "$backup_dir/$(dirname "$rel")" && cp "$tgt" "$backup_dir/$rel" || {
          red "  ✗ failed to stage a rollback copy of $rel"
          _update_rollback_writes "$backup_dir" "${created[@]+"${created[@]}"}"
          return 1
        }
        local has_fm=0; _update_has_fm "$src" && has_fm=1
        local merged
        if merged=$(_update_build_expected "$src" "$tgt" && printf '\034'); then
          merged=${merged%?}
          printf '%s' "$merged" > "$tgt" || {
            red "  ✗ failed to replace $rel"
            _update_rollback_writes "$backup_dir" "${created[@]+"${created[@]}"}"
            return 1
          }
        else
    ...
    if ! bash "$SCRIPT_DIR/cumaru" doctor --quiet; then
      if [[ $gate_was_clean -eq 1 ]]; then
        _update_rollback_writes "$backup_dir" "${created[@]+"${created[@]}"}"
        red "✗ post-update checks failed; every file this update wrote was rolled back"
      else
        yellow "⚠ post-update checks failed, and they already failed before this update; the merged files were kept."
        yellow "→ Reconcile the reported errors, or restore the tree from Git."
      fi
      rm -rf "$backup_dir"
      return 1
    fi
...
cmd_update_artifacts() {
  ...
  while [[ $# -gt 0 ]]; do
    case "$1" in
      ...
      --with=*) with_skills+=("${1#--with=}"); shift ;;
      *) red "unexpected arg: $1"; return 2 ;;
    esac
  done
  if [[ "$kind" != skills && ${#with_skills[@]} -gt 0 ]]; then
    red "✗ --with is supported only by 'cumaru update skills'"
    return 2
  fi
  if [[ $clear -eq 1 && ${#with_skills[@]} -gt 0 ]]; then
    red "✗ --with cannot be combined with --clear"
    return 2
  fi
  ...
  if [[ $clear -eq 1 ]]; then
    ...
    if [[ -z "$requested" ]]; then
      for agent in generic claude codex opencode; do
        _update_clear_artifact_kind "$parent" "$kind" "$agent" || return 1
      done
    else
      _update_clear_artifact_kind "$parent" "$kind" "$agent" || return 1
    fi
    bash "$SCRIPT_DIR/cumaru" doctor --quiet || return 1
  ...
  _update_validate_artifact_source "$source_domain" "$domain" || return $?
  _framework_validate_opt_in_skills "$source_root/skills" "${with_skills[@]+"${with_skills[@]}"}" || return 1
  _agent_describe "$parent" "$agent"
  if [[ $apply -eq 0 ]]; then
    if [[ ${#with_skills[@]} -gt 0 ]]; then
      say "Dry-run only. Re-run with --apply to install opt-in skills for $agent: ${with_skills[*]}."
    else
  ...
  if [[ "$kind" == skills ]]; then
    if [[ ${#with_skills[@]} -gt 0 ]]; then
      _framework_install_opt_in_skills "$parent" "$source_root/skills" "$agent" "${with_skills[@]}" || return 1
    else
    ...
    _framework_prune_deprecated_cumaru_commands "$parent" "$source_domain"
  fi
  bash "$SCRIPT_DIR/cumaru" doctor --quiet || return 1
...
cmd_update_agent() {
  local requested="${1:-}" clear=0 apply=0 arg
  [[ "$requested" == --* ]] && requested=""
  [[ -n "$requested" ]] && shift
  while [[ $# -gt 0 ]]; do
    arg="$1"
  ...
  [[ -f "$CONFIG" && -f "$CUMARU_DIR/index.md" ]] || { red "✗ $CUMARU_DIR is not installed"; return 1; }
  schema_validate_installed "$CONFIG" "$CUMARU_DIR/index.md" || return $?
  local parent a=""; parent=$(dirname "$CUMARU_DIR")
  [[ -z "$requested" ]] || a=$(_agent_normalize "$requested") || { red "✗ unknown agent: $requested"; return 2; }
  if [[ $clear -eq 1 ]]; then
    _update_check_git_recovery "$(cd "$parent" && pwd -P)" || return $?
    if [[ -z "$requested" ]]; then
    ...
    else _agent_remove_adapter "$parent" "$a" "" || return 1; fi
    bash "$SCRIPT_DIR/cumaru" doctor --quiet || return 1
  ...
  local source_domain domain CUMARU_AGENT_OVERRIDE
  _update_check_git_recovery "$(cd "$parent" && pwd -P)" || return $?
  domain=$(schema_get_domain "$CONFIG") || return 1
  source_domain="$SCRIPT_DIR/domains/$( [[ "$domain" == base ]] && printf '%s' __base || printf '%s' "$domain" )"
  [[ -d "$source_domain" ]] || { red "✗ source domain not found: $source_domain"; return 1; }
  _update_validate_artifact_source "$source_domain" "$domain" || return $?
  CUMARU_AGENT_OVERRIDE="$a"
  _framework_install_skills "$parent" "$source_domain" 1 || return 1
  _framework_prune_deprecated_cumaru_skills "$parent" "$source_domain" || return 1
  if [[ "$a" != codex ]]; then
    _framework_copy_commands "$parent" "$source_domain" 1 || return 1
    _framework_prune_deprecated_cumaru_commands "$parent" "$source_domain" || return 1
  fi
...
# Emit v9 update pairs as destination and canonical-source paths. Both trees
# must explicitly mark the same logical entry framework-owned: ownership is
# never inherited from a parent. The shared resolver also rejects malformed
# paths, symlinked entries, and destination collisions before any write.
_update_v9_candidates() {
  local source_domain="$1" target_root="$2" source_rows target_rows
  local logical target_rel ownership ignored source_logical source_rel source_ownership
  local found has_glob

  source_rows=$(mktemp) || return 1
  target_rows=$(mktemp) || { rm -f "$source_rows"; return 1; }
  config_tree_resolve "$source_domain/config.yaml" "$source_domain" > "$source_rows" || { rm -f "$source_rows" "$target_rows"; return 1; }
  config_tree_resolve "$CONFIG" "$target_root" > "$target_rows" || { rm -f "$source_rows" "$target_rows"; return 1; }

  while IFS=$'\t' read -r logical target_rel ownership ignored; do
    [[ "$ownership" == "true" ]] || continue
    found=""
    has_glob=0
    case "$logical" in *[\*\?\[]*) has_glob=1 ;; esac
    while IFS=$'\t' read -r source_logical source_rel source_ownership ignored; do
      [[ "$source_logical" == "$logical" && "$source_ownership" == "true" ]] || continue
      [[ "$has_glob" == "0" || "$source_rel" == "$target_rel" ]] || continue
      found="$source_rel"
      break
    done < "$source_rows"
    [[ -n "$found" ]] || continue
    [[ -f "$source_domain/$found" && ! -L "$source_domain/$found" ]] || {
      red "✗ framework source is missing or unsafe for $logical: $found"
      rm -f "$source_rows" "$target_rows"
      return 1
    }
    printf '%s\t%s\n' "$target_rel" "$found"
  done < "$target_rows"
