...
_update_require_clean_git_worktree() {
  local parent="$1" status
  if ! git -C "$parent" rev-parse --is-inside-work-tree >/dev/null 2>&1; then
    red "✗ cumaru update --apply requires a Git work tree"
    yellow "→ Commit or otherwise create a user-controlled recovery point before running a mutating update."
    return 1
  fi
...
_update_validate_artifact_source() {
  ...
  local source_version source_index_version target_version
  ...
  if [[ "$source_version" != "$source_index_version" || "$source_version" != "$target_version" ]]; then
...
Arguments:
  ...
  agent <name>   preview or directly apply a native agent switch.
...
Options:
  ...
  --apply        rebuild framework Markdown from canonical source while
                   ...
                   agent artifacts after the clean-Git preflight. config.yaml is
...
Transaction safety:
  Every mutating apply requires a Git work tree with no staged, unstaged, or
  untracked changes before Cumaru writes managed files. Commit, stash, or clean
  first; Git history is the recovery boundary.
...
cmd_update() {
  ...
  if [[ $apply -eq 1 ]]; then
    _update_require_clean_git_worktree "$(cd "$parent" && pwd -P)" || return $?
    # .cumaru/ file merge.
    if [[ $total -gt 0 ]]; then
      for rel in "${changed[@]}"; do
        local src="$source_domain/$rel" tgt="$CUMARU_DIR/$rel"
        mkdir -p "$(dirname "$tgt")" || { red "  ✗ failed to create parent for $rel"; return 1; }
        if [[ ! -f "$tgt" ]]; then
          cp "$src" "$tgt" || { red "  ✗ failed to create $rel"; return 1; }
          green "  ✓ created $rel"
          continue
        fi
        local has_fm=0; _update_has_fm "$src" && has_fm=1
        ...
        if merged=$(_update_build_expected "$src" "$tgt" && printf '\034'); then
          merged=${merged%?}
          printf '%s' "$merged" > "$tgt" || { red "  ✗ failed to replace $rel"; return 1; }
        else
