...
# strips frontmatter from both, and inserts domain preservation at the base
# preservation checkpoint before printing the remaining universal sequence.
#
...
#
# Legacy configuration names are resolved here only; runtime v9 uses CONFIG.
...
# Prints the base body before or after the domain-extension checkpoint.
_migrate_base_part() {
  local file="$1" part="$2"
  awk -v part="$part" '
    NR == 1 && $0 == "---" { in_fm = 1; next }
    ...
    in_fm                  { next }
    $0 == "<!-- cumaru:migration-domain-extension -->" { after = 1; next }
    part == "before" && !after { if (!started && $0 == "") next; started = 1; print }
    part == "after" && after   { if (!started && $0 == "") next; started = 1; print }
  ' "$file"
}
...
cmd_migrate() {
  ...
  grep -Fxq '<!-- cumaru:migration-domain-extension -->' "$base_doc" || {
    red "✗ base migration is missing its domain preservation checkpoint"
    return 1
  }
...
EOF
  ...
  _migrate_base_part "$base_doc" before
  ...
  printf '\n'
  _migrate_base_part "$base_doc" after
}
