...
_tree_summary() {
  ...
  if [[ ! "$count" =~ ^[0-9]+$ || "$count" -lt 32 || "$count" -gt 512 ]]; then
    _tree_diag "$rel" "summary must contain 32 to 512 Unicode code points"
    return 1
  fi
...
# Validate the optional domain guard and pillar list against the installed
...
_tree_validate_filters() {
  ...
  [[ -f "$schema" && ! -L "$schema" ]] || {
    printf 'cumaru tree: filters require a regular .cumaru/config.yaml\n' >&2
    return 1
  }

  installed_domain=$(yq -r '.domain | select(tag == "!!str" and length > 0)' "$schema" 2>/dev/null) || {
    printf 'cumaru tree: cannot read domain from .cumaru/config.yaml\n' >&2
    return 1
  }
  if [[ -z "$installed_domain" ]]; then
    printf 'cumaru tree: config domain must be a non-empty string\n' >&2
    return 1
  fi
  ...
  for pillar in "${value[@]}"; do
    ...
    if config_tree_is_v9 "$schema"; then
      declared=$(config_tree_root_keys "$schema" | grep -xF "$pillar" >/dev/null && printf true || printf false)
    else
    ...
    if config_tree_is_v9 "$schema"; then
      local physical
      physical=$(config_tree_root_directory "$pillar") || {
        printf 'cumaru tree: cannot resolve pillar for domain %s: %s\n' \
          "$(_tree_quote_path "$installed_domain")" "$(_tree_quote_path "$pillar")" >&2
        return 1
      }
      [[ ",$normalized," == *",$physical,"* ]] || normalized="${normalized:+$normalized,}$physical"
    else
