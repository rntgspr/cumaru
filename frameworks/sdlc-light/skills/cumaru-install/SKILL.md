...
version: 1
name: cumaru-install
description: Use this skill whenever the user wants to adopt the dot-llm framework in a project — install the .cumaru/ tree, choose a domain, wire the .agents/ instruction file, and (post-install) bootstrap the spec areas for an existing codebase. Trigger on phrases like "install the framework", "set up .cumaru/ here", "adopt dot-llm", "instala o framework", "bootstrap specs from the codebase", "scaffold the spec areas", "compactar / consolidate area X", "deepen the auth spec", or any request to seed/grow the `specs/` pillar. The install itself is deterministic (copy framework files + skills + slash commands); the spec bootstrap that follows is LLM-driven via this skill.
---
