...
3. **Classify each in-scope file**:
   ...
   |---|---|---|
   | Shallow index | path matches a pillar's `index.md` (any direct child dir of `.cumaru/`) | Union both sides' rows then re-emit the pillar tag via `cumaru tag set <pillar>/index.md <pillar> <new body>` to canonicalize the marker block. |
...
6. **Walk** processes files in this order: Shallow index → Marker file → Plain `.cumaru/`. For each:
   ...
   - For Shallow index, re-emit the pillar tag via `cumaru tag set <pillar>/index.md <pillar> <new body>` immediately after to keep the marker block canonical.
