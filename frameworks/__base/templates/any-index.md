...
<!-- This block is the pillar's shallow index — the ONLY thing that enters
     ...
     under root.entities in schema.yaml. The columns below come from that
     declaration's `columns:` list; replace them with the actual column names.
     Rows are added here (one per entry) as items are created in this pillar.
     The `<!-- llm:NAME --> / <!-- /llm:NAME -->` wrapper is required: the CLI
     and LLM use it to locate and update the table. -->
| <Col1> | <Col2> | <Col3> | <...> |
|--------|--------|--------|-------|
