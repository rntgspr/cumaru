...
Use `cumaru tag get/set` (CLI, no skill) for `specs/index.md` table round-trip; pair with `cumaru-plan` (which declares `scope:` paths), `cumaru-archive` (which absorbs deltas), and `cumaru-doctor` to verify post-op.
