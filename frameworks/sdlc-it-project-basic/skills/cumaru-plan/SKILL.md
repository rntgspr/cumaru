...
├── handoff-t1.md     ← [plan!, task!, status!, date!] + <!-- cumaru:files --> ([Link, Description] table — one row per touched file)
├── t2.md             ...
└── delta-draft.md    ← [plan!, status!, date!]  (status: always `draft`)
```

`<PLAN-ID>`:
- Tracker-backed → `<KEY>` (e.g. `AAA-1234`). Requires `intake/<KEY>/` to exist.
...
2. Open `templates/handoff.md`; author the frontmatter:
   ...
   - `date: <YYYY-MM-DD>`.
3. Body sections:
   - `## Files touched` — fill the `<!-- cumaru:files -->` block as a `[Link, Description]` table, one row per file: `| [`<path>`](<path>) | created/modified/removed — one-line description of the change |`. Use `git diff --name-status` to enumerate; the LLM writes the description per file.
...
7. `cumaru doctor` — file refs check should find every link in the handoff's `<!-- cumaru:files -->` table resolves on disk.
...
3. **Hand off to [`cumaru-archive`](../cumaru-archive/SKILL.md)** — it runs the archive recipe (move to `archive/<PLAN-ID>/`, finalize delta, absorb into specs, remove plan dir, re-emit indexes).
...
| User says | You do |
|---|---|
| "Start a plan for AAA-1234" | Tracker-backed bootstrap recipe → confirm scope → create plan + maybe T1 → re-emit table |
