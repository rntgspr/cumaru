...
`archive/<PLAN-ID>/` and `exploring/<slug>/` are **never drilled by default** — only when a spec's `deltas:`, a plan's `key:`, or the user reference them explicitly. See the canonical Loading rule in the root `.cumaru/index.md` for the full per-role table.
...
9. **Commit absorption and prune the archive directory.**
   ...
   - Update `archive/index.md` row with `Absorbed-in: <sha>` (via `cumaru tag set`).
   - `cumaru flow archive/<KEY> remove` and commit `chore(.cumaru): prune archive/<KEY>/ post-absorption`.
