...
- Plan tickets and internal initiatives by writing `plans/<PLAN-ID>/index.md` (frontmatter, scope, Plan/DAG, out-of-scope, risks) and the corresponding `t<N>.md` task files. For tracker-backed plans (`key:` set in frontmatter), Overview and Acceptance Criteria (EARS / RFC 2119) live in `intake/<KEY>/index.md`, not in the plan body.
...
- Run the **archive flow** on plan close: receive the Dev's `delta-draft.md`, validate, finalize as `delta.md`, absorb into specs, move the plan, update index tables.
...
2. If the request maps to a tracker item: ensure `intake/<KEY>/index.md` exists and is fresh (re-sync if stale). The intake file owns `## Overview` and `## Acceptance Criteria (EARS / RFC 2119)` for this ticket — author them in English from the tracker description, refining as understanding sharpens.
...
2. Read the Dev's `plans/<PLAN-ID>/delta-draft.md`. Validate:
   - Every EARS / RFC 2119 criterion is covered by an Added or Modified Requirement (or explicitly noted as not requiring a spec change). For tracker-backed plans the criteria live in `intake/<KEY>/index.md`; for slug-based plans they live in the plan body.
...
8. Regenerate the shallow indexes by hand (until the CLI exists):
   - `archive/index.md` — append the in-flight row while absorption is active.
...
9. **Commit absorption, record it in specs, and clean archive.**
   ...
   - Remove the `<KEY>` row from `archive/index.md`, then `cumaru flow archive/<KEY> remove` and commit `chore(.cumaru): record <KEY> absorption`.
