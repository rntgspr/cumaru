...
> This section documents the original motivation for the framework, drawn from the web/software development workflow where it was first applied. It is preserved here because `dot-llm-framework/` ships a starter tuned for that context (intake → plans → specs cycle, Lead/Dev/Ghost roles, tracker integration). If you are adapting the framework for a different discipline, treat this as reference — the model itself is not software-specific.

Two opposite trade-offs show up across spec-driven approaches for software projects:
...
How this framework differs from common alternatives in the web/software space:
