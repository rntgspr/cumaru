...
4. **Run Phase 1** (non-destructive — copies + frontmatter updates):
   ...
   - Add an in-flight row in `archive/index.md`.
...
6. **Run Phase 3** — commit the spec absorption, capture the SHA, append `SHA | KEY | Description` to `specs/index.md` `cumaru:absorptions`, remove the row from `archive/index.md`, remove `archive/$ARGUMENTS/`, commit cleanup, then run `cumaru doctor`.
...
- **Re-emit every affected index** (`archive/index.md`, `specs/index.md` including `absorptions`, and `plans/index.md` row disappearance) via `cumaru tag set` before declaring done.
