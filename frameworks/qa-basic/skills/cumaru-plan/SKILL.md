...
├── handoff-t1.md     ← [plan!, task!, status!, date!] + <!-- cumaru:files --> ([Link, Description] table — one row per touched spec/fixture/factory)
...
8. Re-emit `plans/index.md` row via `cumaru tag set plans/index.md plans <new body>` — v4 shape: `| [<PLAN-ID>](<PLAN-ID>/index.md) | <one-line description: title, tasks done/total, apps> |`.
9. `cumaru doctor`.
...
5. Update the plan's `## Cases / DAG` table; re-emit `plans/index.md`; `cumaru doctor`.
...
- `## Files touched` — fill the `<!-- cumaru:files -->` block as a `[Link, Description]` table, one row per file: `| [`<path>`](<path>) | created/modified/removed — one-line description of the change |`.
...
- Update the DAG Status; re-emit `plans/index.md`; `cumaru doctor`.
...
| User says | You do |
|---|---|
| "Start a campaign for AAA-1234" | Tracker-backed bootstrap → confirm scope → create plan + maybe T1 → re-emit table |
| "Nova campanha pra cobrir o checkout" | Slug-based bootstrap → propose slug → full body (incl. test strategy) |
| "Add case T3 to AAA-1234" | Add-case recipe → write t3.md (Author/Verify) → update DAG → re-emit |
