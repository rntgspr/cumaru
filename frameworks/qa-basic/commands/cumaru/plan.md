...
4. **Run the recipe.** Walk the steps from the skill, confirming every judgment call (scope = coverage areas the campaign touches; test strategy that respects the pyramid; case breakdown; level distribution; reference relevant `standards/` instead of restating). Use `cumaru flow` for file ops, `cumaru tag set plans/index.md plans <new body>` to re-emit the index row.
...
- Re-emit `plans/index.md` row via `cumaru tag set` after **every** structural change (new campaign, new case, case status flip, close).
