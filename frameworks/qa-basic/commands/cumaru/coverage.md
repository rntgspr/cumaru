...
1. **Read the `cumaru-coverage` skill** from the installed agent skills directory (`.agents/skills/cumaru-coverage/SKILL.md` for Claude or `.agents/skills/cumaru-coverage/SKILL.md` for Codex). It carries the three recipes: bootstrap, deepen, consolidate. Follow its layout and pre-checks (Lead-only authoring; `deltas:` ↔ `consolidated-at:` state model; reference `standards/` instead of restating conventions).
...
3. **Run the recipe.** Walk the skill's steps, confirming every judgment call (area split, concern promotion, scenario completeness, level distribution per pyramid, consolidation cuts). Use `cumaru flow` for file ops and `cumaru tag set coverage/index.md coverage <new body>` to re-emit the index row.

4. **Close out.** Run `cumaru doctor` and report. Surface any new orphan rows/files introduced by structural changes.
...
- **Re-emit `coverage/index.md` row** via `cumaru tag set` after any structural change (new area, new concern, promoted subarea, consolidation).
