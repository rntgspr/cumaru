...
Work flows `intake → plans → archive`, is distilled into the durable layer (`coverage/`), then all transient content for that cycle is removed — the archive row, the plan dir, the related intake items, and the exploring entry. Only `coverage/` (the living coverage map) and `standards/` (testing conventions) remain as durable records.
