---
version: 1
description: Run `cumaru intake <KEY>` to mirror a tracker issue under `.cumaru/intake/<KEY>/`, then refine the generated file per the embedded RAW instructions (Overview, EARS criteria, apps=environments, bug sections) before deleting the block.
...
5. **Walk the refinement, section by section.** Apply the edits to the *body above* the RAW block. Do not touch frontmatter `status:` or `synced-at:` — the CLI manages those.
   ...
   - **`## Acceptance Criteria (EARS)`** — bullets in the form `WHEN <trigger> THE SYSTEM SHALL <observable response>` (also `WHEN <trigger> AND <condition>` and `WHILE <state>`). For infra: criteria often reference observable states (the stack exists in `<env>`, the apply diff is empty, outputs match), not user-facing behavior. Extract from explicit AC when present; otherwise propose criteria derived from the Overview and ask the user to confirm/edit. **Every bullet must conform to the EARS pattern — `cumaru doctor` flags non-conforming bullets as a warning.**
...
9. **Close out.** Print the final path and run `cumaru doctor`. Report: refinement complete, EARS warnings (none expected), `apps:` value applied (or the blocker noted in step 6), and doctor totals.
