...
├── handoff-t1.md     ← [plan!, task!, status!, date!] + <!-- cumaru:files --> ([Link, Description] table — one row per touched HCL/manifest/module)
...
5. Update the plan's `## Plan / DAG` table; re-emit `plans/index.md`; `cumaru doctor`.
...
| User says | You do |
|---|---|
| "Start a change for AAA-1234" | Tracker-backed bootstrap → confirm scope → create plan + maybe T1 → re-emit table |
| "Novo plano de manutenção pra rotacionar os certs" | Slug-based bootstrap → propose slug → full body (incl. blast radius/rollback) |
| "Add step T3 to AAA-1234" | Add-step recipe → write t3.md (Apply/Verify) → update DAG → re-emit |
