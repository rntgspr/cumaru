...
Work flows `intake → plans → archive`, is distilled into the durable layer (`topology/`), then all transient content for that cycle is removed — the archive row, the plan dir, the related intake items, and the exploring entry. Only `topology/` (the living infra model) and `runbooks/` (operational procedures) remain as durable records.
