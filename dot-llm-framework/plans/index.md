...
- **One directory per plan.** `plans/<JIRA-KEY>/` for Jira-backed work, `plans/maintenance-<slug>/` for internal initiatives without a Jira ticket. The directory name is the plan ID.
- **Slug-based plans require the `maintenance-` prefix.** Pure kebab-case slug (`maintenance-cleanup-deprecated-helpers`). No Jira frontmatter field.
- **Plan body for Jira-backed plans carries `## Plan / DAG`, `## Out of scope`, `## Risks` only.** Overview and Acceptance Criteria live in the linked `intake/<type>/<KEY>/index.md` and are not duplicated here. Slug-based plans keep both inside the plan body.
- **Stories are linear at the Lead level.** Only one plan from the same story is active at a time. Cross-ticket coordination happens in the story's `## Coordination` section under `intake/stories/<STORY>/`.
...
- Starting work on a Jira ticket → create `plans/<JIRA-KEY>/` (Lead).
- Internal initiative without a Jira ticket → create `plans/maintenance-<slug>/` (Lead).
...
- Ticket scope, Overview, or Acceptance Criteria → `intake/<type>/<KEY>/`.
