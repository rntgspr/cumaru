...
### Inspect drift without updating state

Select the exact workspace, backend, environment inputs, and provider credentials first. Then inspect only:

```bash
terraform plan -refresh-only
```

This proposes state and root-output changes without committing them. A request
to inspect drift stops here; it does not authorize state reconciliation.

After the proposed state changes are reviewed, reconcile them only with
...
Review the plan shown by `apply` again before approving it. This writes state
and root outputs, even though it does not change remote objects.

Do not use `terraform refresh` for inspection. It is deprecated and behaves as
`terraform apply -refresh-only -auto-approve`: it writes state without an
approval prompt. Terraform added `-refresh-only` in v0.15.4. On an older
version, report that inspection mode is unavailable; never fall back to the
mutating `refresh` command without a separate, explicit authorization.
