...
- During `cypress run`, Cypress saves a failure screenshot unless
  `screenshotOnRunFailure` is disabled. The in-runner **command log** shows
  every step with a time-travel snapshot.
- Keep video recording disabled (`video: false`, the Cypress default). Do not
  enable it unless the user explicitly requests video or the repository already
  requires it.

## Reading a failure

Read the **command log** at the failed step — hover to time-travel the DOM to
that moment. Inspect a failure screenshot when one was produced. The error names
the failed assertion or the command that couldn't find an element. Never claim
that a video exists without checking the runner configuration and output.
...
- **Handoff** — record the `cypress run` summary, the actual screenshot or other
  artifact used to investigate a failure, and the **flakiness check** (ran clean
  N times, or quarantined with the artifact attached). With `video: false`, no
  video artifact is expected.
...
Reference: [Cypress screenshots and videos](https://docs.cypress.io/app/guides/screenshots-and-videos).
