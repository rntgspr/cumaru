...
- Choose mock cleanup by effect:
  - `vi.clearAllMocks()` clears call history without changing implementations.
  - `vi.resetAllMocks()` clears history and resets mock implementations.
  - `vi.restoreAllMocks()` also restores original descriptors for spies created
    with `vi.spyOn()`.
- Mock cleanup does not reset application state, module state, the DOM, timers,
  environment mutations, or test data. Clean those resources explicitly in the
  owning fixture or hook. Use `clearMocks`, `mockReset`, or `restoreMocks` only
  when its exact effect matches the suite's isolation contract.
...
- **`targets:`** on a coverage area / case / task = the levels exercised — `unit`, `integration`.
...
Reference: [Vitest mock API](https://vitest.dev/api/mock).
