...
|---|---|---|---|
| Slogan-directive | eager, always applicable (dilutes) | `domain.md` prose | no |
| **Discipline** | **eager body; application by trigger** | **`disciplines/<name>.md`** | **yes, modular** |
| Skill | eager description + body by trigger | selected adapter's skill directory | yes |

A discipline file is declared in `domain.md` and delivered eagerly through the existing adapter
bootstrap. Its `applies-when:` decides when the rules bind, never whether the body is loaded. It is
native to `.cumaru/` and does not create a second loading mechanism.
...
applies-when: <task condition under which this discipline's rules bind>
strictness: <0/10 through 10/10, where 10/10 is inflexible when applicable>
summary: <32-512 character description of the discipline's rule>
source:
  plugin: <owner/repo>
