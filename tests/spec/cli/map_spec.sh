...
alpha.md	9	Example title'
  ...
  It 'preserves numeric source order for single- and double-digit headings'
    write_raw_md "$TREE_PROJECT/.cumaru/order.md" $'## Line one\n2\n3\n4\n5\n## Line six\n7\n8\n## Line nine\n10\n11\n## Line twelve\n'
    When call cli_in "$TREE_PROJECT" map order.md --rows
    The status should be success
    The output should equal 'order.md	1	Line one
order.md	6	Line six
order.md	9	Line nine
order.md	12	Line twelve'
    The error should be blank
  End

  It 'uses literal heading semantics inside frontmatter'
    ...
    The status should be success
    The output should equal 'frontmatter.md	2	Metadata title
frontmatter.md	5	Document title'
    The error should be blank
  End

  It 'requires ripgrep instead of changing heading semantics through a fallback'
    When run env PATH=/usr/bin:/bin /bin/bash -c 'cd "$1" && exec /bin/bash "$2" map alpha.md --rows' _ "$TREE_PROJECT" "$CLI"
    The status should be failure
    The output should be blank
    The error should equal 'cumaru map: ripgrep (rg) is required'
  End
  ...
  It 'reports descendant symlinks while retaining valid heading rows'
    printf '%s\n' '## Safe title' >> "$TREE_PROJECT/.cumaru/alpha.md"
    printf '%s\n' '## Outside title' > "$TREE_PROJECT/outside.md"
    ln -s "$TREE_PROJECT/outside.md" "$TREE_PROJECT/.cumaru/unsafe.md"
    When call cli_in "$TREE_PROJECT" map --rows
    The status should be failure
    The output should equal 'alpha.md	7	Safe title'
    The error should include 'cumaru map: unsafe.md: symlinks are not supported'
  End

  It 'reports control-character descendants while retaining valid heading rows'
    printf '%s\n' '## Safe title' >> "$TREE_PROJECT/.cumaru/alpha.md"
    control_path="$TREE_PROJECT/.cumaru/invalid"$'\t'"name.md"
    printf '%s\n' '## Unsafe title' > "$control_path"
    When call cli_in "$TREE_PROJECT" map --rows
    The status should be failure
    The output should equal 'alpha.md	7	Safe title'
    The error should include 'path contains a control character'
