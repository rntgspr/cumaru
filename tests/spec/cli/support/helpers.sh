...
write_doctor_md() {
  ...
  mkdir -p "$(dirname "$path")"
  {
    printf '%s\n' '---' 'human_revised: false' "summary: '$summary'" 'generated: false' 'targets: [meta]'
    [ -z "$extra" ] || printf '%s\n' "$extra"
...
fs_setup() {
  FS_TMP=$(mktemp -d "${TMPDIR:-/tmp}/cumaru-fs-spec.XXXXXX")
  FS_PROJECT="$FS_TMP/project"
  mkdir -p "$FS_PROJECT"
  cp -R "$REPO_ROOT/tests/fixtures/fs/.cumaru" "$FS_PROJECT/.cumaru"
}

fs_cleanup() { rm -rf "$FS_TMP"; }

fs_snapshot() { (cd "$FS_PROJECT" && find .cumaru -print | LC_ALL=C sort && cksum .cumaru/plans/item/index.md .cumaru/plans/item/note.md); }
...
rules:
  ...
  index_md: { frontmatter: [generated!, targets!] }
  pillar_index: { frontmatter: [generated!, targets!] }
root:
  frontmatter: [framework-version!, depends-on]
  entities:
    notes:
      entities:
        note: { path: <slug>.md, frontmatter: [generated!, targets!] }
meta:
  targets: { values: [meta] }
  tags:
...
doctor_setup() {
  ...
  write_doctor_md "$DOCTOR_PROJECT/.cumaru/index.md" 'Root navigation contract for the focused doctor fixture.' $'framework-version: 8\ndepends-on: [domain.md]'
