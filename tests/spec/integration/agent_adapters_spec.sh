...
Describe 'stateless agent artifact integration'
  ...
  It 'installs matching OpenCode skills and argument-forwarding commands'
    ...
    When run bash -c '
      test -f "$1/.agents/skills/cumaru-resolve/SKILL.md" &&
      test -f "$1/.opencode/commands/cumaru/resolve.md" &&
      grep -Fq '\''Arguments: `$ARGUMENTS`'\'' "$1/.opencode/commands/cumaru/resolve.md" &&
      grep -Fq '\''Load the installed `cumaru-resolve` skill'\'' "$1/.opencode/commands/cumaru/resolve.md"
    ' _ "$project"
    The status should be success
  End
  ...
  It 'clears commands without removing instructions or skills'
    project="$INTEGRATION_TMP/project"; install_base "$project"
    git_clean_baseline "$project" || return 1
    run_in_project "$project" "$INTEGRATION_ROOT/cumaru" update agent claude --apply >/dev/null
