...
make_update_source() {
  ...
  yq -i ".meta.targets.values = [\"${marker}-source\", \"meta\"]" "$source/domains/__base/config.yaml"
}
...
make_update_project() {
  ...
  git -C "$project" add . >/dev/null 2>&1 || return
  git -C "$project" -c user.name=Cumaru -c user.email=cumaru@example.invalid commit -m 'baseline' >/dev/null 2>&1 || return
}

commit_update_project() {
  local project="$1" message="${2:-checkpoint}"
  git -C "$project" add -A >/dev/null 2>&1 || return
  git -C "$project" -c user.name=Cumaru -c user.email=cumaru@example.invalid commit -m "$message" >/dev/null 2>&1 || return
}
