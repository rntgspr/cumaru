...
Describe 'update integer version gate'
  ...
  gate_config() {
    ...
    printf '%s\n' "version: $version" "domain: $domain" 'agent: null' 'rules:' '  markdown:' '    required_heading: h1' '    frontmatter: [human_revised!, summary!]' '  index_md:' '    frontmatter: [targets!]' '  pillar_index:' '    frontmatter: [targets!, summary!]' 'root:' '  frontmatter: [depends-on]' '  tags: {}' '  entities: {}' 'meta:' '  targets:' '    values: [meta]' '  tags: {}' '  compatibility:' '    framework_version_field: framework-version' '    framework_version_location: .cumaru/index.md' '    rule: [Legacy v8 fixture compatibility.]' > "$file"
  }
  gate_index() {
    ...
    printf '%s\n' 'generated: false' 'targets: [meta]' '---' '' '# Kernel' >> "$file"
  }
  ...
  It 'does not require legacy root version frontmatter'
    source="$UPDATE_TMP/source"; project="$UPDATE_TMP/project"; make_gate_source "$source" 5 0; make_gate_project "$project" 5 0
    When call run_cumaru "$project" update --from "$source"
    The status should equal 0
    The output should include 'Target: .cumaru (config version 5)'
  End
