...
Describe 'cumaru update content and configuration ownership'
  ...
  It 'reports config drift without mutating or creating a persistent backup'
    yq -i '.meta.targets.values = ["custom", "meta"] | .root.obsolete = {"optional": true}' "$PROJECT/.cumaru/config.yaml"
    cp "$PROJECT/.cumaru/config.yaml" "$UPDATE_TMP/before.yaml"
  ...
  It 'reports deprecated archive surfaces without deleting them'
    rm -rf "$PROJECT"
    mkdir -p "$PROJECT"
    run_cumaru "$PROJECT" install --domain sdlc-full >/dev/null || return 1
    mkdir -p "$PROJECT/.cumaru/archive" "$PROJECT/.agents/skills/cumaru-archive"
    printf '%s\n' 'legacy evidence' > "$PROJECT/.cumaru/archive/evidence.txt"
    printf '%s\n' 'legacy skill' > "$PROJECT/.agents/skills/cumaru-archive/SKILL.md"
    yq -i '.root.archive = {"optional": true}' "$PROJECT/.cumaru/config.yaml"
    printf '%s\n' '---' 'human_revised: false' 'summary: Legacy archive evidence retained for deliberate adopter reconciliation.' 'targets: [meta]' '---' '# Archive' > "$PROJECT/.cumaru/archive/index.md"

    When call run_cumaru_combined "$PROJECT" update --from "$UPDATE_ROOT"
    The status should equal 0
    The output should include 'deprecated archive artifact: .cumaru/archive/'
    The output should include 'deprecated archive configuration: .cumaru/config.yaml root.archive'
    The output should include 'deprecated archive artifact: .agents/skills/cumaru-archive'
    The output should include 'general update will not delete or migrate it'
    The contents of file "$PROJECT/.cumaru/archive/evidence.txt" should equal 'legacy evidence'
    The contents of file "$PROJECT/.agents/skills/cumaru-archive/SKILL.md" should equal 'legacy skill'
  End
End
